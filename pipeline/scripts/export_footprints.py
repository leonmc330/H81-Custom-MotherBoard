...
OUTPUT_DIR  = "../footprints.pretty"

PIN_SCALE   = 2.54 * 0.0001   # TVW units â†’ mm (same as pintokicad.py)
FLIP_X      = True
FLIP_Y      = True
...
# =========================
# DEDUPLICATE â€” one footprint per unique device name.
# When multiple refdes share the same device, use the one with the most pins.
# =========================

device_best = {}   # device_name â†’ (ref, comp)
...
for device in sorted(device_best.keys()):
    ...
    if filename in used_files:
        print(f"  COLLISION: {device!r} â†’ {filename} already written, skipping")
        collisions += 1
        continue
    ...
    lines.append(f'  (fp_line (start  {hx:.3f}  {hy:.3f}) (end {-hx:.3f}  {hy:.3f}) (layer "F.CrtYd") (width {CYD_W}))')
    lines.append(f'  (fp_line (start {-hx:.3f}  {hy:.3f}) (end {-hx:.3f} {-hy:.3f}) (layer "F.CrtYd") (width {CYD_W}))')
    # Pads â€” un-rotate world coords into footprint-local space
