...
OUTPUT_FILE     = "../kicad/output.kicad_pcb"
PININFO_FILE    = "../input/pininfo.txt"

PIN_SCALE       = 2.54 * 0.0001   # micrometers â†’ mm
FLIP_X          = True
FLIP_Y          = True
...
print(f"  Pin count distribution: " + "  ".join(
    f"{n}pinÃ—{c}" for n, c in sorted(pin_counts.items())))
...
with open(COMP_FILE, newline="") as f:
    for row in csv.reader(f):
        ...
        #   refdes, [deviceâ€¦], x, y, sizex, sizey, rotation   (7+ CSV columns)
        # Old format (no rotation column):
        #   refdes, [deviceâ€¦], x, y, sizex, sizey              (6+ CSV columns)
        # Device name may contain commas â†’ parse from the RIGHT.
...
print(f"  Rotation distribution: " + "  ".join(
    f"{r}Â°Ã—{c}" for r, c in sorted(rotation_dist.items())))

# Cross-reference pins â†” components
...
if all_x:
    ...
    print(f"\nBoard extents: {board_w:.1f} Ã— {board_h:.1f} mm  "
          f"(center at {center_x:.0f}, {center_y:.0f} TVW units)")
else:
    center_x = center_y = 0.0
    print("\nWARNING: no pin data â€” board extents unknown")
...
def load_pininfo():
    ...
    try:
        ...
        print(f"\nINFO: {PININFO_FILE} not found â€” using default pad size {PAD_SIZE_DEFAULT} mm for all pads")
...
# =========================
# PASS 1 â€” PIN FOOTPRINTS (pads only, no outlines)
...
nets_per_comp = []
largest_comps = []
pin_centroids = {}  # refdes â†’ (at_x, at_y) in mm â€” reused by pass 2 for logging
...
# =========================
# PASS 2 â€” OUTLINE FOOTPRINTS (courtyard only, no pads)
# Positioned at the component's anchor from components.csv, NOT the pin centroid.
# sizex/sizey are component-local â€” swap for 90Â°/270Â° to match world orientation.
# =========================

print(f"Pass 2: skipped â€” courtyard outlines are now embedded in pass-1 pin footprints")
...
for n_pins, ref in largest_comps[:10]:
    dev = components[ref]["device"] if ref in components else "â€”"
